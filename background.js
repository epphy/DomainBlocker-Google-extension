let blockedDomainList = [];

chrome.storage.onChanged.addListener(() => updateBlockedDomainList());

function updateBlockedDomainList() {
    chrome.storage.sync.get(["blockedDomainList"]).then((response => blockedDomainList = response.blockedDomainList || []));
}

updateBlockedDomainList();
